...
const DATABASE_NAME = 'NHD_ASMR-data-s1';
const STORE_NAME = 'DataStore';
const TIMESTAMP_KEY = 'timestamp';
const DATA_KEY = 'data';
const DATA_PATH = '/assets/data/s1.csv';
const cacheDuration = {
    development: 10000,
    production: 24 * 60 * 60 * 1000
}
const cacheDurationApply = cacheDuration.development;
...
async function load() {
    await buildDatabaseWithCSV(cacheDurationApply);
    Database.completeBuild();
}

load().then(() => {
    const event = new CustomEvent('databaseBuilt');
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            if (!db.objectStoreNames.contains(STORE_NAME)) {
                ...
                store.createIndex(TIMESTAMP_KEY, TIMESTAMP_KEY);
            }
        };
...
async function saveToIndexedDB(data, timestamp) {
    ...
    store.put(timestamp, TIMESTAMP_KEY);
...
async function getFromIndexedDB() {
    ...
    return new Promise((resolve, reject) => {
        ...
        const dataRequest = store.get(DATA_KEY);
        const timestampRequest = store.get(TIMESTAMP_KEY);

        let data, timestamp;
        let completedRequests = 0; // Cờ để kiểm tra các yêu cầu đã hoàn thành
...
async function fetchAndProcessCSV() {
    try {
        ...
        console.error('Error while fetch or handle CSV file:', error);
        return [];
    }
}

async function buildDatabaseWithCSV(cacheDuration) {
    const currentTime = Date.now();
    const cachedData = await getFromIndexedDB();

    if (cachedData?.data && cachedData.timestamp && (currentTime - cachedData.timestamp < cacheDuration)) {
        console.log('Using cached data from IndexedDB.');
        cachedData.data.forEach(values => {
            at(...values); // Gọi hàm at với mỗi dòng dữ liệu
        });
    } else {
        console.log('Fetching and processing CSV...');
        const processedData = await fetchAndProcessCSV();
        processedData.forEach(values => {
            at(...values); // Gọi hàm at với mỗi dòng dữ liệu
        });

        // Lưu dữ liệu vào IndexedDB
        await saveToIndexedDB(processedData, currentTime);
